...
logging.basicConfig(level=logging.INFO, filename='eu2019_exporting_topics.log', filemode='a', format='%(asctime)s - %(levelname)s - %(name)s - %(message)s')
...
filename = "exported_tweets_topics_post.json"
...
for row in uniqueTopics:
    ...
    print(row)
    for topic in row:
        n = db.eu2019v3.count_documents({'$and':[{'lang': lang},{'$text':{'$search': '\"'+topic+'\"'}}]})
...
with pysftp.Connection(host=myHostname, username=myUsername, password=myPassword, cnopts=cnopts) as sftp:
    ...
    localFilePath = r"exported_tweets_topics_post.json"
